...
EDGE_ROUTING_MAP = {
    # 补回战机与挂载方案的连线！
    "DataAircraftLoadouts": "aircraft_can_equip",
    "DataAircraftSensors": "aircraft_has_sensor",
    "DataShipSensors": "ship_has_sensor",
    "DataSubmarineSensors": "submarine_has_sensor",
    "DataGroundUnitSensors": "groundunit_has_sensor",
    "DataFacilitySensors": "facility_has_sensor",
    "DataSatelliteSensors": "satellite_has_sensor",
    "DataMountSensors": "mount_has_sensor",
    "DataWeaponSensors": "weapon_has_sensor",
    "DataAircraftMounts": "aircraft_has_mount",
    "DataShipMounts": "ship_has_mount",
    "DataSubmarineMounts": "submarine_has_mount",
    "DataGroundUnitMounts": "groundunit_has_mount",
    "DataFacilityMounts": "facility_has_mount",
    "DataSatelliteMounts": "satellite_has_mount",
    "DataMountWeapons": "mount_can_fire",
    "DataWeaponWarheads": "weapon_has_warhead",
    "DataShipMagazines": "ship_has_magazine",
    "DataSubmarineMagazines": "submarine_has_magazine",
    "DataFacilityMagazines": "facility_has_magazine",
    "DataGroundUnitMagazines": "groundunit_has_magazine",
    "DataAircraftComms": "aircraft_has_comm",
    "DataShipComms": "ship_has_comm",
    "DataSubmarineComms": "submarine_has_comm",
    "DataGroundUnitComms": "groundunit_has_comm",
    "DataFacilityComms": "facility_has_comm",
    "DataSatelliteComms": "satellite_has_comm",
    "DataWeaponComms": "weapon_has_comm",
    "DataMountComms": "mount_has_comm",
    "DataShipAircraftFacilities": "ship_has_aircraft_facility",
    "DataFacilityAircraftFacilities": "facility_has_aircraft_facility",
    "DataSubmarineAircraftFacilities": "submarine_has_aircraft_facility",
    "DataGroundUnitAircraftFacilities": "groundunit_has_aircraft_facility",
    "DataShipDockingFacilities": "ship_has_docking_facility",
    "DataFacilityDockingFacilities": "facility_has_docking_facility",
    "DataSubmarineDockingFacilities": "submarine_has_docking_facility",
    "DataGroundUnitDockingFacilities": "groundunit_has_docking_facility",
    "DataAircraftPropulsion": "aircraft_has_propulsion",
    "DataShipPropulsion": "ship_has_propulsion",
    "DataSubmarinePropulsion": "submarine_has_propulsion",
    "DataGroundUnitPropulsion": "groundunit_has_propulsion",
    "DataWeaponPropulsion": "weapon_has_propulsion",
    "DataAircraftFuel": "aircraft_has_fuel",
    "DataShipFuel": "ship_has_fuel",
    "DataSubmarineFuel": "submarine_has_fuel",
    "DataGroundUnitFuel": "groundunit_has_fuel",
    "DataFacilityFuel": "facility_has_fuel",
    "DataWeaponFuel": "weapon_has_fuel",
    # 微观消耗品关联
    "DataAircraftSignatures": "aircraft_has_signature",
    "DataShipSignatures": "ship_has_signature",
    "DataSubmarineSignatures": "submarine_has_signature",
}
...
def establish_schema(session, plantuml_path, tags):
    ...
    run_ngql(
        session,
        "CREATE EDGE IF NOT EXISTS `loadout_contains`(DefaultLoad int, MaxLoad int, Optional bool, Internal bool);",
    )
    run_ngql(
        session, "CREATE EDGE IF NOT EXISTS `magazine_contains_weapon`(Quantity int);"
    )
...
def pump_data(sqlite_path, plantuml_path, space_name):
    ...
    run_ngql(
        session,
        f"CREATE SPACE IF NOT EXISTS {space_name} (partition_num=10, replica_factor=1, vid_type=fixed_string(64));",
    )
    time.sleep(15)
    ...
    for rel_table, edge_name in EDGE_ROUTING_MAP.items():
        ...
        for key in [
            ...
            if key in rel_table:
                tgt_table = (
                    f"Data{key[:-1]}"
                    if key.endswith("s") and key not in ["Comms"]
                    else f"Data{key}"
                )
                if key == "Comms":
                    tgt_table = "DataComm"
        ...
        src_table = rel_table.replace(tgt_table[4:] + "s", "").replace(
            tgt_table[4:], ""
        )
    ...
    try:
        cursor.execute(
            "SELECT lw.ID, wr.ComponentID, wr.DefaultLoad, wr.MaxLoad, lw.Optional, lw.Internal FROM DataLoadoutWeapons lw JOIN DataWeaponRecord wr ON lw.ComponentID = wr.ID"
        )
        count, values_list = 0, []
        for row in cursor.fetchall():
            values_list.append(
                f'"Loadout_{row[0]}"->"Weapon_{row[1]}":({row[2] or 0}, {row[3] or 0}, {"true" if row[4] else "false"}, {"true" if row[5] else "false"})'
            )
            count += 1
            if count % 200 == 0:
                run_ngql(
                    session,
                    f"INSERT EDGE `loadout_contains`(DefaultLoad, MaxLoad, Optional, Internal) VALUES {', '.join(values_list)};",
                )
                values_list = []
        if values_list:
            run_ngql(
    ...
    try:
        ...
        qty_col = (
            "MaxLoad"
            if "MaxLoad" in cols
            else ("Quantity" if "Quantity" in cols else "1")
        )
        ...
        for row in cursor.fetchall():
            ...
            if count % 200 == 0:
                run_ngql(
                    session,
                    f"INSERT EDGE `magazine_contains_weapon`(Quantity) VALUES {', '.join(values_list)};",
                )
                values_list = []
        if values_list:
            run_ngql(
    ...
    for tag in [
        ...
        run_ngql(
            session,
            f"CREATE TAG INDEX IF NOT EXISTS idx_{tag.lower()} ON {tag}();",
            ignore_error=True,
        )
    time.sleep(5)
    for tag in [
